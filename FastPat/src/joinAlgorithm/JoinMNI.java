...
public class JoinMNI<NodeType, EdgeType> {
	...
	private DFSCode<NodeType, EdgeType> code;
	...
	public JoinMNI(Query qry, DFSCode<NodeType, EdgeType> code) {
		...
		this.code = (DFSCode<NodeType, EdgeType>) code;// P: DFSCode format
		this.nodeLabelMap = qry.getNodeLabelMap();
		esMap = new HashMap<Integer, PatternEdges>();
	}

	public JoinMNI(Graph singleGraph, HashMap<String, Integer> patternEdgeMap) {
		this.KG = singleGraph;
	...
	public IntFrequency joinMNI() {
		IntFrequency frq = new IntFrequency(0);
		this.qrySize = qryGraph.getNodeCount();

		System.out.println("before join UB: " + UB.intValue());
		...
		for (Entry<Integer, PatternEdges> pesEntry : allEdgeTables.entrySet()) {
			...
			if (Settings.earlyTermination && freq < currentMinFreq) {
				System.out.println("@@@@@ early termination @@@@@");
				return new IntFrequency(freq);
			}
		}

		// group by node labels
		HashMap<Integer, HashSet<Integer>> superMap = new HashMap<Integer, HashSet<Integer>>();
		for (Entry<Integer, PatternEdges> e : allEdgeTables.entrySet()) {
			PatternEdges pe = e.getValue();
			int nodeA = pe.getIndexA();
			HashSet<Integer> nodeASet = pe.nodeASet();
			if (!superMap.containsKey(nodeA)) {
				superMap.put(nodeA, nodeASet);
			} else {
				HashSet<Integer> tmp = superMap.get(nodeA);
				if (tmp.size() > nodeASet.size()) {
					superMap.put(nodeA, nodeASet);
				}
			}
			int nodeB = pe.getIndexB();
			HashSet<Integer> nodeBSet = pe.nodeBSet();
			if (!superMap.containsKey(nodeB)) {
				superMap.put(nodeB, nodeBSet);
			} else {
				HashSet<Integer> tmp = superMap.get(nodeB);
				if (tmp.size() > nodeBSet.size()) {
					superMap.put(nodeB, nodeBSet);
				}
			}
		}

		// tighter upper bound after join operation
		UB = new IntFrequency(superMap.get(0).size());
		for (Entry<Integer, HashSet<Integer>> entry : superMap.entrySet()) {
			HashSet<Integer> superNode = entry.getValue();
			int tmp = superNode.size();
			if (UB.intValue() > tmp) {
				UB = new IntFrequency(tmp);
			}
		}
		if (Settings.earlyTermination && UB.intValue() <= currentMinFreq) {
			System.out.println("@@@@@ early termination @@@@@");
			return UB;
		}
		System.out.println("after every two tables UB: " + UB.intValue());
...
//		easyTupleReduceTest();
		...
		while (edgesMap != null && edgesMap.size() != 0) {
			...
			tau = traverseTable(table, tau, domSize, allEdgeTables);
		}
...
//			nodePairs = tupleReduceEdge(edge);// TODO
			ArrayList<int[]> sampleNodePairs = edge.tupleReduceCR(qrySize);// reduce tuples in the table
			reducedEdge.setListOnly(sampleNodePairs);
			reducedTables.put(reducedEdge.getEdgeIdx(), reducedEdge);
		}
		
		HashMap<Integer, PatternEdges> reducedMap = new HashMap<Integer, PatternEdges>();
		reducedMap.putAll((HashMap<Integer, PatternEdges>) reducedTables.clone());// <edgeIdx, patternEdges>
		...
		// single join other full tables
		for (Entry<Integer, PatternEdges> pesEntry : reducedMap.entrySet()) {
			PatternEdges sampleTable = pesEntry.getValue();//reduced table
			InsTable reducedTable = new InsTable();
			ArrayList<int[]> nPairs = sampleTable.getNodePairs();
			for (int i = 0; i < nPairs.size(); i++) {
				int arr[] = nPairs.get(i);
				// initialize the instance by pattern size
				Instances instance = new Instances(qrySize);
				instance.assign(sampleTable.getIndexA(), arr[0]);// (pattern nodeIdx, node)
				instance.assign(sampleTable.getIndexB(), arr[1]);
			...
			pesMap.remove(sampleTable.getEdgeIdx());
			while (pesMap != null && pesMap.size() != 0) {
				// adjacent table of pe
				PatternEdges adjEdge = getAdjacentTable(reducedTable, pesMap);
				if (adjEdge == null || reducedTable.getSize() == 0)
					break;
				pesMap.remove(adjEdge.getEdgeIdx());
				// if join success then next
				reducedTable = priorJoinTables(reducedTable, adjEdge);
			}
			// Mark all values in sample table to the corresponding domains
			if (reducedTable.getSize() != 0) {
				for (int i = 0; i < reducedTable.getSize(); i++) {
					Instances ins = reducedTable.get(i);
					ReTable.addItem(ins);
				}
				markNodes(ReTable);
			}
	...
	public InsTable easyTupleReduceTest() {
		InsTable ReTable = new InsTable();
		// reduce each edge table T in PT
		HashMap<Integer, PatternEdges> reducedTables = new HashMap<Integer, PatternEdges>();
		for (Entry<Integer, PatternEdges> allEntry : qry.getPEMap().entrySet()) {
...
//			nodePairs = tupleReduceEdge(edge);// TODO
//			reducedEdge.setList(nodePairs);
//			System.out.println("edgeIdex:"+reducedEdge.getEdgeIdx());
//			System.out.println("size:"+nodePairs.size());
//			reducedTables.put(reducedEdge.getEdgeIdx(), reducedEdge);
			
			PatternEdges edge = allEntry.getValue();
			PatternEdges reducedEdge = new PatternEdges();
			ArrayList<int[]> nodePairs = tupleReduceEdge(edge);// TODO
			reducedEdge.setListOnly(nodePairs);
			reducedTables.put(edge.getEdgeIdx(), reducedEdge);
			System.out.println("size:"+nodePairs.size());
		}
		...
		for (Entry<Integer, PatternEdges> pesEntry : qry.getPEMap().entrySet()) {
			...
			HashMap<Integer, PatternEdges> pesMap = new HashMap<Integer, PatternEdges>();
			pesMap.putAll((HashMap<Integer, PatternEdges>) qry.getPEMap().clone());// <edgeIdx, patternEdges>
			ArrayList<int[]> nodePairs = tupleReduceEdge(table);// TODO
			System.out.println("size2:"+nodePairs.size());
			...
			for (int i = 0; i < nodePairs.size(); i++) {
				...
				// initialize the instance by pattern size
				Instances instance = new Instances(qrySize);
				instance.assign(table.getIndexA(), arr[0]);// (pattern nodeIdx, node)
				instance.assign(table.getIndexB(), arr[1]);
...
//			ArrayList<int[]> nodePairs2 = reducedTables.get(table.getEdgeIdx()).getNodePairs();
...
//			System.out.println("RTable size:"+RTable.getSize());
...
//				System.out.println("RTable size:"+RTable.getSize()+" adjEdge size:"+adjEdge.getNodePairs().size());
			...
			if (RTable.getSize() != 0) {
				code.setTupleSize(RTable.getSize());
				markNodes(RTable);
			}
		...
		if (domainMap.size() != 0) {
			...
			if (minOfTable < domSize) {
				...
				if (!hasTraverseNodes.contains(anotherNode)) {
					...
					table.changeMinNode();
					tau = traverseTable(table, tau, minOfTable, allEdgeTables);
				} else
					return tau;
			}
		}
